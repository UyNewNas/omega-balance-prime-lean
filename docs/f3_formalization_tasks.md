...
### DEN-1 当前 stacked 候选：精确层与尾层

基于 PR #22 exact head `6922dad91bf4301d3bbbf6b78f4233b5f6677635` 的已验证未加权 AP 计数桥，后续 stacked 分支已加入：幂三 AP 差分极限及常数 `1/3^k`；全体素数上的 `F₃=±k` 嵌套余数刻画（显式处理 2、3）；实际 `F₃=±k` 素数计数与两层 AP 计数差的精确恒等式；以及 `|F₃|≥K` 作为模 `3^K` 的 `±1` 两个互斥余数类之并的尾部候选。当前只把 exact-head 全绿的层登记完成；运行中/失败的后代不提前标记。DEN-2、LOG、RUN 不受此 stacked 分支替代。
