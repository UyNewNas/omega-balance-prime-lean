...
`primeFactorSum` sums the prime-factor list. It is NOT `omegaSum`.
All signed differences and defects are integers. The value at zero is
mathlib's totalized value, not a usual prime factorization of zero.
-/
...
/-- Simultaneous sum balance and count balance at a specified level. -/
...
theorem primeFactorSum_mul {a b : ℕ} (ha : a ≠ 0) (hb : b ≠ 0) :
  ...
  simpa [primeFactorSum, List.sum_append] using (Nat.perm_primeFactorsList_mul ha hb).sum_eq
...
/-- A factorization certificate may contain repeated primes. -/
theorem primeFactorSum_of_factors {n : ℕ} {l : List ℕ}
    (hprod : l.prod = n) (hprime : ∀ p ∈ l, p.Prime) : primeFactorSum n = l.sum := by
  exact (Nat.primeFactorsList_unique hprod hprime).sum_eq.symm

theorem bigOmega_of_factors {n : ℕ} {l : List ℕ}
    (hprod : l.prod = n) (hprime : ∀ p ∈ l, p.Prime) : bigOmega n = l.length := by
...
theorem primeFactorSumDiff_eq_half_diff {n : ℕ} (hn : 1 < n) (hodd : n % 2 = 1) :
  ...
  simpa only [hl, hr, primeFactorSumDiff, neighborDiff] using primeFactorSum_double_diff ha hb

theorem primeFactorSumDiff_prime_eq_half_diff {p : ℕ} (hp : p.Prime) (h2 : 2 < p) :
...
theorem primeFactorSumBalanced_iff_half {n : ℕ} (hn : 1 < n) (hodd : n % 2 = 1) :
...
theorem primeFactorSum_center_iff {m : ℕ} (hm : 0 < m) :
  ...
  have hl : (2 * m + 1 - 1) / 2 = m := by omega
  have hr : (2 * m + 1 + 1) / 2 = m + 1 := by omega
  rw [hl, hr]

/-- A general cofactor construction with explicit primality hypotheses. -/
...
/-- Composite q,r are allowed in the exact defect identity. -/
