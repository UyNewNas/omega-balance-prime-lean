...
/-!
# Formal-power-series bridge prototypes for the genuine 3-adic F₃ logarithm

These kernel-checked examples validate the exact pinned-mathlib interfaces
needed to turn the explicitly summed project logarithm into evaluation of
`PowerSeries.log ℚ_[3]`, and likewise identify the analytic exponential
with evaluation of `PowerSeries.exp ℚ_[3]`.

They intentionally introduce no public theorem declaration until the bridge
proof scripts have passed the repository build; public interfaces are added
only together with their `scripts/Audit.lean` entries.
-/
